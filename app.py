...
try:
    ...
    with tab_year:
        ...
        # Courbe Année N (Dorée, plus épaisse)
        fig_season.add_trace(go.Scatter(
            x=MOIS_FR, 
            y=stats_n.values, 
            name=f"Année {year_target}",
            line=dict(color='#D4AF37', width=4),
            mode='lines+markers+text',
            text=stats_n.values, # Affiche le chiffre sur le point
            textposition="top center"
        ))

        # Courbe Année N-1 (Beige, pointillée)
        fig_season.add_trace(go.Scatter(
            x=MOIS_FR, 
            y=stats_n1.values, 
            name=f"Année {year_target-1}",
            line=dict(color='#E5D3B3', width=2, dash='dot'),
            mode='lines+markers'
        ))
        ...
        fig_season.update_layout(
            ...
            yaxis=dict(
                ...
                showgrid=True,
                gridcolor='rgba(200, 200, 200, 0.3)', # Quadrillage léger
                dtick=50, # Un marqueur principal toutes les 50 unités
                minor=dict(ticklen=4, dtick=10, showgrid=True, gridcolor='rgba(200, 200, 200, 0.1)') # Lignes secondaires toutes les 10 unités
            ),
    ...
    with tab_month:
        ...
        if df_m.empty:
            ...
            with col_left:
                ...
                # Création de tranches de délais
                bins = [-1, 3, 7, 10, 15, 1000]
                labels = ['Express (0-3j)', 'Standard (4-7j)', 'Tendu (8-10j)', 'Retard (11-15j)', 'Critique (>15j)']
                # On s'assure que DELAI est numérique et sans NaN pour le calcul
                df_temp = df_m.dropna(subset=['DELAI']).copy()
                if not df_temp.empty:
                    df_temp['TRANCHE_DELAI'] = pd.cut(df_temp['DELAI'], bins=bins, labels=labels)
                    fig_delai = px.bar(df_temp['TRANCHE_DELAI'].value_counts().reindex(labels), 
                                       color_discrete_sequence=['#D4AF37'])
                    fig_delai.update_layout(showlegend=False, height=350, xaxis_title="", yaxis_title="Nombre de pièces")
                    st.plotly_chart(fig_delai, use_container_width=True)
                else:
                    st.write("Pas de données de délais disponibles (dates de retour manquantes).")

            with col_right:
                st.subheader("👗 Top 5 Modèles retouchés")
                top_models = df_m['NOM ARTICLE'].value_counts().head(5).reset_index()
                top_models.columns = ['Modèle', 'Volume']
                # Utilisation d'une palette séquentielle qui existe : 'YlOrBr' (Yellow-Orange-Brown)
                fig_models = px.pie(top_models, values='Volume', names='Modèle', hole=0.4,
            ...
            # --- 3. RÉCAPITULATIF OPÉRATIONNEL ---
            st.subheader("📑 Détail des opérations du mois")
            search = st.text_input("Rechercher un client, une souche ou un article...", placeholder="Tapez ici...")
   ...
   # --- TAB 3 : SUIVI DES FLUX ---
    with tab_flux:
        ...
        # Définition des seuils temporels
        un_an_ago = datetime.now() - timedelta(days=365)
        un_mois_ago = datetime.now() - timedelta(days=30)
        
        # 1. ALERTES CRITIQUES (> 1 AN)
        alertes_critiques = df[(df['DATE DISPO'].notna()) & 
                               (df['RECUPERE'] == False) & 
                               (df['DATE DISPO'] < un_an_ago)].copy()
        
        if not alertes_critiques.empty:
            st.error(f"🚨 **ALERTE CRITIQUE : {len(alertes_critiques)} retouches sont en boutique depuis plus d'un AN !**")
            st.write("Ces pièces devraient probablement être sorties du stock ou faire l'objet d'une procédure spécifique.")
            st.dataframe(
                alertes_critiques[['DATE DISPO', 'NOM', 'N° SOUCHE', 'NOM ARTICLE']]
                .sort_values('DATE DISPO'), 
                use_container_width=True
            )
            st.markdown("---")

        # 2. ALERTES RELANCES CLASSIQUES (> 30j et < 1 an)
        alertes_stock = df[(df['DATE DISPO'].notna()) & 
                           (df['RECUPERE'] == False) & 
                           (df['DATE DISPO'] < un_mois_ago) &
                           (df['DATE DISPO'] >= un_an_ago)].copy()

        if not alertes_stock.empty:
            st.warning(f"⚠️ **{len(alertes_stock)} retouches sont en boutique depuis plus de 30 jours.**")
            st.dataframe(alertes_stock[['DATE DISPO', 'NOM', 'N° SOUCHE', 'NOM ARTICLE']].sort_values('DATE DISPO'), use_container_width=True)
            
            # Bouton de téléchargement combiné pour les relances
            full_relances = pd.concat([alertes_critiques, alertes_stock])
            csv = full_relances.to_csv(index=False).encode('utf-8')
            st.download_button("📩 Télécharger la liste complète des relances (inclus +1 an)", csv, "relances_globales.csv", "text/csv")
        else:
        ...
        # 3. TOUTES LES RETOUCHES EN ATTENTE (Chez le retoucheur)
